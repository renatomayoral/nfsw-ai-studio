...
export async function GET(req: NextRequest) {
  ...
  const rows = await db.select().from(platform).orderBy(asc(platform.sortOrder), asc(platform.label))
...
const createSchema = z.object({
  key: z.string().min(1).max(40).regex(/^[a-z0-9_-]+$/, 'Apenas letras minúsculas, números e hífens'),
...
export async function POST(req: NextRequest) {
  ...
  if (existing.length) return NextResponse.json({ error: 'Já existe uma plataforma com essa chave' }, { status: 409 })
